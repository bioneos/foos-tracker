function EmbOverTimeGraph(config)
{
  // Private variables
  ...
  //var line = d3.line().curve(d3.curveMonotoneX)
    ...
    .y(function(d) { return yScale(d.embs); });

  // Public methods:
  // Our graph type identifier (read only)
  this.type = function() { return 'EoT'; };
  ...
  this.updateWidth = function(newWidth) {
    ...
    player.select(".nick")
      ...
      .attr("transform", function(d) { 
        return "translate(" + xScale(new Date(d.value.when)) + "," + yScale(d.value.embs) + ")"; 
      });
  };
  ...
  function updateData(gamesData, callback)
  {
    ...
    // Adjust our scales
    var yMax = d3.max(playersData.entries(), function(d) { return d.value[d.value.length - 1].embs; });
    ...
    .transition().duration(TRANSITION_DURATION)
      .attr("transform", function(d) { 
        return "translate(" + xScale(new Date(d.value.when).getTime()) + "," + yScale(d.value.embs) + ")"; 
      })
    ...
    gamesData.games.forEach(function(game) {
      d3.keys(game.goals).forEach(function (nick) {
        ...
        if (!playersData.has(nick)) 
          ...
          total = playersData.get(nick)[playersData.get(nick).length - 1].embs || 0;
        playersData.get(nick).push({ when: game.when, embs: total + (game.goals[nick] === 0 ? 1 : 0)});
      });
    });
